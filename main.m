...
load('frcnnimg.mat');
label=bwlabel(sout);
stats=regionprops(logical(sout),'MajorAxisLength','Solidity','Area','BoundingBox');
Volume1=[stats.MajorAxisLength];
area=[stats.Area];

%disp(X);
volume = sum(label(:));
TumourVolume=volume
high_dense_area=Volume1>0.7;
...
if max_area>200
   figure;
   imshow(tumor)
   title('Alzhimer alone','FontSize',20);
else
    h = msgbox('No Alzheimer!!','status');
    %disp('no Alzheimer');
    return;
end
...
%% Getting  Outline - image filling, eroding, subtracting
...
figure;  
imshow(tumorOutline);
title('Alzheimer Outline','FontSize',20);
...
title('Detected Alzhimer','FontSize',20);
...
subplot(234);imshow(tumor);title('Alzheimer alone','FontSize',20);
subplot(235);imshow(tumorOutline);title('Alzheimer Outline','FontSize',20);
subplot(236);imshow(tumorOutlineInserted);title('Detected Alzhimer','FontSize',20);
